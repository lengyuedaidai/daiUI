...
layui.define('layer', function(exports) {
			...
			items = {
				...
				select: function() {
						...
						selects.each(function(index, select) {
							...
							//替代元素
							var reElem = $(['<div class="layui-unselect ' + CLASS + (disabled ? ' layui-select-disabled' : '') + '">', '<div class="' + TITLE + '"><input type="text" placeholder="' + (select.options[0].innerHTML ? select.options[0].innerHTML : TIPS) + '" value="' + (value ? selected.html() : '') + '" ' + (isSearch ? '' : 'readonly') + ' class="layui-input layui-unselect' + (disabled ? (' ' + DISABLED) : '') + '">', '<i class="layui-edge"></i></div>', '<dl class="layui-anim layui-anim-upbit' + (othis.find('optgroup')[0] ? ' layui-select-group' : '') + '">' + function(options) {
								var arr = [];
	...
	var submit = function() {
		...
		layui.each(fieldElem, function(_, item) {
			...
			if(/^checkbox$/.test(item.type) && field[item.name]) {
